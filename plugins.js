...
"sourceUrl": `${link}`}}})
  ...
  Mariabody = `┌──⊰𝘽𝙞𝙨 𝙯𝙪𝙢 𝙣ä𝙘𝙝𝙨𝙩𝙚𝙣 𝙈𝙖𝙡⊰
│⊳  👤 From: ${metadata.subject}
│⊳  📃 Grund: Hat sich vom Staub gemacht
│⊳  📔 Name: @${MariaName.split("@")[0]}
│⊳  👥 Mitglieder: ${Mariamembers}th
...
const Mariadate = moment.tz('Europe/Berlin').format('DD/MM/YYYY')
let MariaName = num
Mariabody = ` 𝐆𝐥ü𝐜𝐤𝐰𝐮𝐧𝐬𝐜𝐡🎉 @${MariaName.split("@")[0]}, Du bist jetzt ein *Admin* 🥳`
   Maria.sendMessage(anu.id,
 { text: Mariabody,
 contextInfo:{
 mentionedJid:[num],
...
const Mariadate = moment.tz('Europe/Berlin').format('DD/MM/YYYY')
let MariaName = num
Mariabody = `*Lol Du wurdest degradiert*‼️ @${MariaName.split("@")[0]}, Was hast du angestellt?`
Maria.sendMessage(anu.id,
 { text: Mariabody,
 contextInfo:{
 mentionedJid:[num],
