...
/////log
global.ownernumber = '4915678394727' 
//Gallery/database
...
Maria.sendMessage(from, {text:`\`\`\`「 Link Erkannt 」\`\`\`\n\n@${m.sender.split("@")[0]} Wurde wegen eines Regelverstoß entfernt.`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
  ...
  let smallinput = budy.toLowerCase()
                ...
                reply(`Aloha Guten Tag und Salam Aleykum❤️ 
                ${pushname}, Wie war dein Tag?`)

                  }



if (smallinput.includes(`Lol`)) {reply(`Lol`)}
                  
                  
if (smallinput.includes(`Hurensohn`)) {

                  reply(`werd Erwachsen du Opfer`)

                  }
                  
                  
                  if (smallinput.includes(`Guten Morgen`)) {

                  reply(`Guten Morgen ${pushname}', Hab einen tollen Tag❤️`)

                  }

        if (smallinput.includes(`Gute Nacht`)) {

                  reply(`Gute Nacht❤️${pushname}', bis morgen,und ich wünsche dir ein kühles Kissen❤️`)

                  }

                          if (smallinput.includes(`Lasse wann Leak`)) {

                  reply(`WANN VERDAMMT NOCHMAL WANN`)

                  }                
                  
                                                              if (smallinput.includes(`bot info`)) {

                  reply(`Hey ich bin ღƐ×ͥΐզͣօͫή BØ₸ღ, ein Whatsapp Bot erstellt von ღƐ×ͥΐզͣօͫήღ, vielleicht bin ich nicht derjenige, den du gefragt hast, aber ich möchte nur, dass du weißt, dass ich ein Bot bin, schreibe .menu um alle verfügbaren Befehle zu sehen.`)

                  }

       if (smallinput.includes(`Exiqon`)) {
       ...
       if (smallinput.includes(`Bot`)) {
         ...
         reply(`Hey mein Name ist ꧁ღƐ×ͥΐզͣօͫή BØ₸ღ꧂.\nSchreibe 😊👉${prefix}menu um meine Befehle kennenzulernen`)

                  }     

//bot_nachricht und antwort
      if (/^bot$/i.test(smallinput)) {
  //antwort
         reply(`Hey mein Name ist ꧁ღƐ×ͥΐզͣօͫή BØ₸ღ꧂.\nSchreibe 😊👉${prefix}menu um meine Befehle kennenzulernen`)

                  }     

       if (smallinput.includes(`@4915678394727`)) {

                  reply(`Wie kann ich dir behilflich sein?`)

                  }
                 
       if (smallinput.includes(`@49491741711168`)) {

                  reply(`sorry der Besitzer ist gerade Beschäftigt teile uns gerne dein Anliegen mit, Wir werden uns dann so schnell wie möglich bei dir melden!`)

                  }


       if (smallinput.includes(`Heyy`)) {

                  reply(`Heyy wie gehts dir?`)

                  }

       if (smallinput.includes(`Guten Morgen`)) {

                  reply(`Guten Morgen ${pushname}', Hab einen tollen Tag😘`)
		...
		if (smallinput.includes(`Thanks`)) {
                  ...
                  reply(`Kein Problem😽😽 ${pushname}`)

                  }
                  
                  
                  if (smallinput.includes(`was geht`)) {
                  ...
                  reply(`WANN VERDAMMT WANN ${pushname}`)
...
Maria.sendMessage(from, {text: `\`\`\`「 ⚠️Warnung⚠️ 」\`\`\`\n\nWenn du kein Admin bist, sende keine Links in diese Gruppe, sonst wirst du sofort gekickt!`, contextInfo: { mentionedJid : mems }}, {quoted:m})
} else if (args[0] === "off") {
...
break
            ...
            case 'join':
                try {
                    ...
                    reply('Gruppenbeitritt Fehlgeschlagen! Bitte überprüfe den Gruppenlink.')
                }
                break      
            case 'session':
                if (!isCreator) return reply(mess.owner)
...
if (!AntiNsfw) return reply('Bereits Deaktiviert✅️')
...
case 'apk':
case 'apkdl':{
if (!text) return reply("Which apk u wanna download?")
